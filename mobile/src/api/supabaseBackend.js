...
function dataUrlToBlob(dataUrl) {
  ...
  return { bytes, mime, _rn: true }
}

/** Upload a data-URL or remote http(s) image; return a stable public URL. */
async function uploadRentalImage(rentalId, fileKey, dataUrl) {
  ...
  } else if (dataUrl.startsWith('file:') || dataUrl.startsWith('content:')) {
  ...
  const { error } = await sb.storage.from('rentals').upload(path, body, {
    contentType,
    upsert: true,
  })
  if (error) throwSb(error)
...
async function materializeCarPhotos(rentalId, carPhotos) {
  ...
  const safeUpload = async (fileKey, dataUrl) => {
    ...
    const needsUpload =
      ...
      dataUrl.startsWith('content:')
    if (!needsUpload) return dataUrl || ''
    try {
  ...
  if (Array.isArray(out.extras)) {
    out.extras = await Promise.all(
      out.extras.map(async (item, index) => {
        ...
        if (typeof item.uri !== 'string') return item
